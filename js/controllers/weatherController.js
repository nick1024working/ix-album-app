import { SunsetRecord } from '../models/SunsetRecord.js';

/**
 * WeatherController：用於從中央氣象局 API 取得資料的控制器。
 *
 * @param {Object} view - 處理畫面顯示的物件。
 * @param {Function} view.showSunset - 必填。當資料成功取得時呼叫，傳入解析後的資料。
...
export class WeatherController {
    ...
    async fetchSunset(date) {
        try {
            ...
            url.searchParams.set('parameter', 'SunSetTime');
            const res = await fetch(url);
            const data = await res.json();

            const record = new SunsetRecord(data);
